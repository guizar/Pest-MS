...
# ipm = ipm value associated for each crop, if NA means that no forecast is included in this observation
...
# country_percentiles =  percentile position associated with this country, for each crop
# ////
...
# join SUMMARY_OUTLIERS with YLD_CROP_STATS data (add tonnes as  country_tonnes, and percentiles as country_percentiles)
# rename YLD_CROP_STATS vars
YLD_CROP_STATS$country_tonnes = YLD_CROP_STATS$tonnes
YLD_CROP_STATS$country_percentiles = YLD_CROP_STATS$percentiles
...
cols = c("LON","LAT","NAME","region","crop","phi","ipm","tonnes","country_tonnes","country_percentiles")
...
fg = ggplot(data = gg, 
            aes(x=ipm, 
                ...
                color=tonnes, shape=crop), 
            group=NAME)
fg = fg + geom_point(position = position_identity(),alpha=0.5)
